...
if 'Dem_Mayor' in df.columns:
    # Three-tier imputation to recover obs where mayor_pid is missing but
    # alternative partisan info is available. Adds `Dem_Mayor_imputed`
    # indicator for transparency; robustness columns can exclude these.
    n_before = df['Dem_Mayor'].notna().sum()
    df['Dem_Mayor_imputed'] = 0

    # Tier 1: Fill from mayor_party where mayor_pid was missing (data engineering
    # gap — mayor_party comes from a separate source that covers some cities
    # that mayor_pid missed).
    if 'mayor_party' in df.columns:
        ...
        log(f"  Tier 1 (mayor_party fallback): filled {int((t1_d|t1_r).sum())} obs "
            f"({int(t1_d.sum())} D, {int(t1_r.sum())} R)")

    # Tier 2: Within-city forward/back fill (covers single-year gaps like
    # 2025 where the latest coding hasn't been entered but prior years are coded).
    ...
    log(f"  Tier 2 (within-city ffill/bfill): filled {int(t2_filled.sum())} obs")

    # Tier 3: Nonpartisan cities with no mayor-level partisan data.
    # Use constituency proxy (pres_dem_share > 0.5) for cities with formally
    # nonpartisan rotating mayors where partisan coding is unavailable.
    ...
    if 'pres_dem_two_party_share_lag2' in df.columns:
        ...
        log(f"  Tier 3 (constituency proxy for nonpartisan cities): filled {int(t3_mask.sum())} obs")

    n_after = df['Dem_Mayor'].notna().sum()
    log(f"  Dem_Mayor total: {n_before} → {n_after} (+{n_after - n_before} recovered)")
